 ...
 *  - copper    = --primary（铜橙 #EA6D15，R3 令牌修正为 25 84% 50% 后
 *    回归变量取色，不再硬编码 '234,109,21'；浏览器精算 ≈ 235,110,20，
 *    与旧硬编码逐通道差 ≤1，无视觉跳变）
 */
...
export type CanvasTheme = {
  ...
  copper: string; // --primary 的 'r,g,b' 三元组（铜橙强调色）
};
...
export function readCanvasTheme(): CanvasTheme {
  ...
  const primary = hslVarToRgb(cs.getPropertyValue('--primary') || '25 84% 50%');
  return {
    isDark,
    main: fg,
    rgb: rgbTriplet(fg),
    bgRgb: rgbTriplet(bg),
    copper: rgbTriplet(primary),
  };
}
